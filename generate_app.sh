...
echo "🚀 ERDINPLAYER (SMARTERS STYLE) OLUŞTURULUYOR..."
...
# 4. APP BUILD.GRADLE (Dependencies Eklendi)
...
# 5. MODEL SINIFLARI (Xtream Verilerini Tutmak İçin)
...
public class LoginResponse {
    ...
    public class ServerInfo {
        ...
        @SerializedName("https_port") public String httpsPort;
        @SerializedName("server_protocol") public String serverProtocol;
    }
}
EOF

# StreamItem.java (Kanal Listesi İçin)
...
public class ChannelAdapter extends RecyclerView.Adapter<ChannelAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        // TV Kumandası Focus Olayı
        holder.itemView.setOnFocusChangeListener((v, hasFocus) -> {
            if(hasFocus) v.setBackgroundColor(context.getResources().getColor(R.color.colorAccent));
            else v.setBackgroundColor(context.getResources().getColor(R.color.card_bg));
        });
    }
...
# 7. API SERVICE (Dinamik URL Desteği)
...
# 8. UTILS (Oturum Bilgilerini Saklamak İçin)
...
# LoginActivity (GERÇEK XTREAM DOĞRULAMA)
...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btnLogin).setOnClickListener(v -> {
            ...
            if(!d.startsWith("http")) d = "http://" + d; // http ekle
            ...
            // 1. Bizim Panele Log At
            logToPanel(d, u, p);

            // 2. Xtream Giriş Kontrolü
    ...
    void performXtreamLogin(String d, String u, String p) {
        try {
            // Retrofit base URL zorunlu oldugu icin gecici veriyoruz, asil islem @Url ile yapilacak
            ...
            api.xtreamLogin(apiPath, u, p).enqueue(new Callback<LoginResponse>() {
                public void onResponse(Call<LoginResponse> call, Response<LoginResponse> response) {
                    if(response.body() != null && response.body().userInfo != null && response.body().userInfo.auth == 1) {
                        // Başarılı! Kaydet ve Dashboard'a git
                ...
                public void onFailure(Call<LoginResponse> c, Throwable t) {
                    Toast.makeText(LoginActivity.this, "Sunucu Hatası: " + t.getMessage(), Toast.LENGTH_LONG).show();
                }
            });
...
# DashboardActivity (SMARTERS MENÜ)
...
public class DashboardActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvUser.setText("Hoşgeldin, " + session.getUser());

        LinearLayout btnLive = findViewById(R.id.btnLive);
        LinearLayout btnMovies = findViewById(R.id.btnMovies); // Şimdilik Live ile aynı yere gitsin
        ...
        btnMovies.setOnClickListener(v -> {
            // Filmler için de aynı mantık, sadece action=get_vod_streams olacak
            // Şimdilik demo
        });
    }
}
...
# LiveListActivity (KANAL LİSTESİ)
...
public class LiveListActivity extends AppCompatActivity {
    ...
    void fetchChannels(String d, String u, String p) {
        try {
            ...
            api.getLiveStreams(apiPath, u, p, "get_live_streams").enqueue(new Callback<List<StreamItem>>() {
                public void onResponse(Call<List<StreamItem>> call, Response<List<StreamItem>> response) {
                    ...
                    if(response.body() != null) {
                        ChannelAdapter adapter = new ChannelAdapter(LiveListActivity.this, response.body(), item -> {
                            // Kanal Tıklanınca Oynat
                            Intent intent = new Intent(LiveListActivity.this, PlayerActivity.class);
                            // Xtream Stream URL: http://dns:port/live/user/pass/streamID.ts
                ...
                public void onFailure(Call<List<StreamItem>> c, Throwable t) {
                    progressBar.setVisibility(View.GONE);
                    Toast.makeText(LiveListActivity.this, "Liste alınamadı", Toast.LENGTH_SHORT).show();
                }
            });
        } catch(Exception e){ e.printStackTrace(); }
    }
...
# 10. XML TASARIMLARI (LAYOUTS)

# item_channel.xml (Liste Elemanı)
...
# activity_list.xml (Liste Sayfası)
...
# activity_dashboard.xml (ANA MENÜ)
...
<LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <LinearLayout android:layout_width="match_parent" android:layout_height="wrap_content" android:gravity="center">
        <LinearLayout android:id="@+id/btnLive"
        ...
        <LinearLayout android:id="@+id/btnMovies"
            android:layout_width="150dp" android:layout_height="120dp"
    ...
    <LinearLayout android:layout_width="match_parent" android:layout_height="wrap_content" android:gravity="center">
        <LinearLayout android:layout_width="150dp" android:layout_height="120dp"
            android:background="#222" android:orientation="vertical" android:gravity="center"
            android:focusable="true" android:clickable="true" android:layout_margin="10dp">
            <TextView android:text="DİZİLER" android:textColor="#FFF" android:textStyle="bold" android:textSize="18sp"/>
        </LinearLayout>
        
        <LinearLayout android:layout_width="150dp" android:layout_height="120dp"
            android:background="#222" android:orientation="vertical" android:gravity="center"
            android:focusable="true" android:clickable="true" android:layout_margin="10dp">
            <TextView android:text="AYARLAR" android:textColor="#FFF" android:textStyle="bold" android:textSize="18sp"/>
        </LinearLayout>
    </LinearLayout>
...
# activity_splash.xml, activity_login.xml, activity_player.xml (Standart)
...
<LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <EditText android:id="@+id/etDns" android:hint="DNS (örn: http://dns.com:8080)" 
...
# 12. MANIFEST (Tüm Aktiviteler Eklendi)
...
echo "✅ ERDINPLAYER (SMARTERS EDITION) TAMAMLANDI."
